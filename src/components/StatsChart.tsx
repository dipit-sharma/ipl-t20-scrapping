...
const StatsChart: React.FC<StatsChartProps> = ({ pointsTable }) => {
  ...
  const teamPerformanceData: TeamPerformance[] = pointsTable.map((team) => ({
    ...
    losses: team.lost,
    winPercentage:
      team.matches > 0 ? Math.round((team.won / team.matches) * 100) : 0,
    averageScore: Math.floor(Math.random() * 50) + 150,
    totalRuns: Math.floor(Math.random() * 500) + 1500,
    totalWickets: Math.floor(Math.random() * 50) + 100,
  }));
  ...
  const renderTrendsChart = () => {
    const trendData = pointsTable.map((team, index) => ({
      ...
      nrr: parseFloat(team.netRunRate) * 10 + 10,
    }));
  ...
  const chartButtons = [
    {
      ...
      icon: BarChart3,
      description: "Team points and wins comparison",
    },
    {
      key: "performance",
      label: "Win Percentage",
      icon: PieChartIcon,
      description: "Team win percentage distribution",
    },
    {
      key: "players",
      label: "Top Players",
      icon: Users,
      description: "Leading run scorers and their stats",
    },
    {
      key: "trends",
      label: "Team Trends",
      icon: TrendingUp,
      description: "Points and NRR trends",
    },
  ];
